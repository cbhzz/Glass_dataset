...
# 输入和输出栅格数据路径
path = r"E:\Tibet Plateau\dataset2\Glass_Albedo_1km\2001"
path_new = r"E:\Tibet Plateau\dataset3\Glass_Albedo_1km\2001"
...
def reproject_and_clip_raster(raster_file, output_file, target_proj_wkt, vector_file):
    ...
    if nodata_value is None:
        nodata_value = -9999  # 如果没有无效值，使用默认值
    ...
    if target_proj_wkt is None:
        ...
        for bn in band_name:
            for i in range(1, 365, 8):
                d2 = os.path.join(path, bn, f"day_{i}.tif")  # 栅格数据文件路径
                d3 = os.path.join(path_new, bn, f"day_{i}.tif")  # 重投影并裁剪后的栅格数据文件路径
